...
class CalculatorServiceTest {
    ...
    public void powerTest() {
        ...
        assertEquals(0.0, calculatorService.power(0, 5));
        assertEquals(1.0, calculatorService.power(1, 100));
        
        // Potentiation with negative exponent
        assertEquals(0.5, calculatorService.power(2, -1));
        assertEquals(0.25, calculatorService.power(2, -2));
        
        // Potentiation with negative base
        assertEquals(4.0, calculatorService.power(-2, 2));
        assertEquals(-8.0, calculatorService.power(-2, 3));
        assertEquals(1.0, calculatorService.power(-1, 0));
    }

    @Test
    public void powerZeroToNegativeExponentTest() {
        ArithmeticException exception = assertThrows(ArithmeticException.class, () -> {
            calculatorService.power(0, -1);
        });
        assertEquals("Zero cannot be raised to a negative power.", exception.getMessage());
    }
}